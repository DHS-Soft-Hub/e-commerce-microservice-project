...
if [ "$1" = "--initial" ]; then
  ...
  if [ -d "Auth.Api/Data/Migrations" ]; then
    rm -rf Auth.Api/Data/Migrations/*
  ...
  dotnet ef migrations add InitialMigration --project Auth.Api --output-dir Data/Migrations
  ...
  # Make sure to run this script from the root of the Auth.Api project
  dotnet ef migrations add "$2" --project Auth.Api --output-dir Data/Migrations
